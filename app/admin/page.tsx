...
type CmsProperty = { id: string; slug: string; name: string; category: string; locality: string; micro_market: string; location: string; price: string; configuration?: string; carpet_area_sqft?: number; image_url?: string; status: string };
...
export default function AdminPage() {
  ...
  async function loadCmsInventory() { const response = await authenticatedFetch(apiUrl("/api/inventory?admin=1")); const payload = await readJson(response); if (!response.ok) return setCmsError(payload.error || "Could not load CMS inventory"); setCmsProperties(payload.data || []); }
  async function createCmsProperty(event: FormEvent) { event.preventDefault(); setCmsBusy(true); setCmsError(""); try { const response = await authenticatedFetch(apiUrl("/api/inventory"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...cmsForm, carpet_area_sqft: cmsForm.carpet_area_sqft ? Number(cmsForm.carpet_area_sqft) : null }) }); const payload = await readJson(response); if (!response.ok) throw new Error(payload.error || "Could not create property"); setCmsProperties((items) => [payload.data, ...items]); setCmsForm({ slug: "", name: "", category: "residential", locality: "Bandra West", micro_market: "Bandra West", location: "", price: "", configuration: "", carpet_area_sqft: "", image_url: "" }); } catch (error) { setCmsError(error instanceof Error ? error.message : "Could not create property"); } finally { setCmsBusy(false); } }
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-section cms-section"><div className="section-title"><div><p className="eyebrow">Chariot CMS</p><h2>Kapil’s inventory</h2><p className="admin-muted">Create a draft, add an image URL or uploaded image, then approve and publish it to the website.</p></div></div><form className="cms-form" onSubmit={createCmsProperty}><input placeholder="Property name" value={cmsForm.name} onChange={(e) => setCmsForm({ ...cmsForm, name: e.target.value })} required /><input placeholder="Slug" value={cmsForm.slug} onChange={(e) => setCmsForm({ ...cmsForm, slug: e.target.value })} required /><input placeholder="Location" value={cmsForm.location} onChange={(e) => setCmsForm({ ...cmsForm, location: e.target.value })} required /><input placeholder="Price / ask" value={cmsForm.price} onChange={(e) => setCmsForm({ ...cmsForm, price: e.target.value })} required /><input placeholder="Configuration (e.g. 3 BHK)" value={cmsForm.configuration} onChange={(e) => setCmsForm({ ...cmsForm, configuration: e.target.value })} /><input placeholder="Carpet sqft" type="number" value={cmsForm.carpet_area_sqft} onChange={(e) => setCmsForm({ ...cmsForm, carpet_area_sqft: e.target.value })} /><input placeholder="Image URL (or upload API)" value={cmsForm.image_url} onChange={(e) => setCmsForm({ ...cmsForm, image_url: e.target.value })} /><button type="submit" className="dark-button" disabled={cmsBusy}>{cmsBusy ? "Saving…" : "Save draft"}</button></form>{cmsError && <p className="error-note">{cmsError}</p>}<div className="cms-list">{cmsProperties.map((property) => <div className="cms-row" key={property.id}><div>{property.image_url && <img src={property.image_url} alt="" />}<strong>{property.name}</strong><small>{property.location} · {property.price}</small></div><span className="pill">{property.status}</span>{property.status !== "published" && <button type="button" className="light-button" onClick={() => setCmsStatus(property.id, "published")}>Publish to website</button>}</div>)}{!cmsProperties.length && <p className="empty-state">No database-backed inventory yet. The public site is using its safe static fallback until the migration is applied.</p>}</div></section>
